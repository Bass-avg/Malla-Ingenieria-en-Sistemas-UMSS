...
*   **Visualización Dinámica:** Malla curricular organizada por semestres (1 al 9).
*   **Lógica de Prerrequisitos:** Las materias se bloquean/desbloquean automáticamente según las materias aprobadas (Clearance logic).
*   **Persistencia de Datos:** El progreso se guarda localmente en el navegador (Local Storage).
*   **Diseño Premium:** Estética *Dark Leather*, fuentes técnicas (`Special Elite`, `Oswald`) y efectos visuales de alta gama.
*   **Control Total:** Botón de reset para limpiar la base de datos de progreso.
...
```bash
├── index.html          # Estructura principal y entry point
├── assets/
...
*   **Fuentes:** `Special Elite` (estilo máquina de escribir) y `Oswald` (sans-serif técnica).
*   **Paleta:**
    *   `--bat-black`: `#0a0a0a` (Profundidad)
    *   `--bat-red`: `#8b0000` (Acento táctico)
    *   `--bat-red-bright`: `#e60000` (Alerta/Aprobado)
